...
export class ScanService {
  ...
  private buildGameEntries(systemPath: string, romFiles: string[], xmlGames: any[], mediaFiles: string[]) {
    ...
    xmlGames.forEach(xmlGame => {
        ...
        // Check Media
        const checkMedia = (xmlPath: string | undefined) => {
            if (!xmlPath) return false;
            let p = xmlPath;
            if (p.startsWith('./')) p = p.substring(2);
            // Media paths in XML can be relative to system rom folder or ~/.emulationstation
            // We assume relative to system folder for this simple scanner
            return fs.existsSync(path.resolve(systemPath, p));
        };
        ...
        entries.push({
            ...
            romExists,
            inGamelist: true,
            imageExists: checkMedia(xmlGame.image),
            videoExists: checkMedia(xmlGame.video),
