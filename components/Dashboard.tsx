...
export const Dashboard: React.FC<DashboardProps> = ({ stats }) => {
  const Card = ({ title, value, icon: Icon, colorClass }: any) => (
    <div className="bg-white dark:bg-retro-800 p-4 rounded-lg border border-gray-200 dark:border-retro-700 flex items-center gap-4 transition-colors duration-200 shadow-sm dark:shadow-none">
      <div className={`p-3 rounded-full bg-gray-100 dark:bg-retro-900 ${colorClass}`}>
        <Icon size={24} />
      </div>
      <div>
